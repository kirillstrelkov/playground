...
def test_json_attributes() -> None:
    datas = json.loads(read_content(os.path.join(os.path.dirname(__file__), "car.json")))
    for data in datas:
        ...
        for attr in data["attributes"]:
            ...
            assert attr_type in supported_types, f"Wrong type {attr_type} in {adac_id} {name}"

            if attr_type in NUMERIC_TYPES and attr["value"]:
                assert "min" in column_data["range"] and "max" in column_data["range"], (
