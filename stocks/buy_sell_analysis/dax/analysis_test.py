...
def test_best_month_day() -> None:
    ...
    assert (
        df_jan[df_jan[Column.DAY] == 1][Column.PERCENT].mean() > df_jan[df_jan[Column.DAY] == 27][Column.PERCENT].mean()
    )
...
def test_best_weekday() -> None:
    ...
    assert not df.empty
    assert df[df[Column.WEEKDAY] == 4][Column.PERCENT].mean() < df[df[Column.WEEKDAY] == 2][Column.PERCENT].mean()


def test_best_hour() -> None:
    df = get_best_hour(FILENAME, YahooRange.DAYS_58, limit=LIMIT)
    assert not df.empty
    assert df[df[Column.HOUR] == 15][Column.PERCENT].mean() > df[df[Column.HOUR] == 13][Column.PERCENT].mean()


def test_best_15mins() -> None:
    df = get_best_quarter(FILENAME, YahooRange.DAYS_58, limit=LIMIT)
    assert not df.empty
    assert df[df[Column.QUARTER] == 45][Column.PERCENT].mean() > df[df[Column.QUARTER] == 15][Column.PERCENT].mean()
...
def test_best_time_hour_and_minute() -> None:
    ...
    assert not df.empty
    assert df[df[Column.TIME] == 15.0][Column.PERCENT].mean() > df[df[Column.TIME] == 13.0][Column.PERCENT].mean()
