...
def __get_feature_df(df):
    ...
    new_df[COLUMN_FEATURE_RANGE] = new_df[COLUMN_FEATURE].apply(lambda x: __get_range(df, x))
...
def __process_initial_data(df) -> None:
...
def test_features_are_up_to_date(df_adac, df_features) -> None:
    df_feature_new = __get_feature_df(df_adac)
    assert not (set(df_feature_new[COLUMN_FEATURE].to_list()).difference(set(df_features[COLUMN_FEATURE].to_list())))


def test_all_features(df_features, df_adac) -> None:
...
def test_all_features_reviews(df_features) -> None:
